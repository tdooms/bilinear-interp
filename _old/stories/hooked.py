...
class Attention(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.hook_q = HookPoint() # [batch seq n_head d_head]
        self.hook_k = HookPoint() # [batch seq n_head d_head]
        self.hook_v = HookPoint() # [batch seq n_head d_head]
        self.hook_z = HookPoint() # [batch seq n_head d_head]
...
class BLP(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.hook_pre = HookPoint()     # [batch seq d_model]
        self.hook_left = HookPoint()    # [batch seq d_hidden]
        self.hook_right = HookPoint()   # [batch seq d_hidden]
        self.hook_post = HookPoint()    # [batch seq d_model]
    ...
    def forward(self, x: Float[Tensor, "batch seq d_model"]) -> Float[Tensor, "batch seq d_model"]:
        ...
        self.hook_left(left)
        self.hook_right(right)
        
        out = self.drop(self.o(left * right))
        self.hook_post(out)
        return out
...
class Transformer(PreTrainedModel):
    ...
    def prepare_inputs(self, dataset):
        ...
        return collator(tokenized["input_ids"])
