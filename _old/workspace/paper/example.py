...
inp = test.x[321]
acts = einsum(inp.flatten(), vecs, "inp, cls comp inp -> cls comp").pow(2) * vals

classes = mnist(inp)[0].topk(3).indices.sort().values.cpu().tolist()

values, idxs = acts[classes].sort(dim=-1)

colors = px.colors.qualitative.Plotly
marker = lambda i: dict(color=colors[i])
...
fig.add_scatter(y=values[0, -12:].flip(0).cpu(), showlegend=False, marker=marker(0), row=1, col=1, mode="lines")
fig.add_scatter(y=values[1, -12:].flip(0).cpu(), showlegend=False, marker=marker(1), row=1, col=1, mode="lines")
